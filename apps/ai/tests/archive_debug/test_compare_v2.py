"""
Test v2 - Generate Antoine and Marjorie with new corrections for comparison.
"""
import time
...
def process_cv(cv_path: Path, output_dir: Path):
    """Process a single CV and save v2 outputs."""
    cv_name = cv_path.stem
    print(f"\n{'=' * 80}")
    print(f"PROCESSING V2: {cv_name}")
    print(f"{'=' * 80}\n")
    ...
    try:
        ...
        # Save PDFs with v2 suffix
        fr_pdf_path = output_dir / f"{cv_name}_fr_v2.pdf"
        en_pdf_path = output_dir / f"{cv_name}_en_v2.pdf"
...
def main():
    """Process CVs for v2 comparison."""
    print(f"\n{'=' * 80}")
    print(f"V2 COMPARISON TEST: {len(CV_FILES)} CVs")
    ...
    print(f"\nCompare v2 with v1 in: {OUTPUT_DIR}")
